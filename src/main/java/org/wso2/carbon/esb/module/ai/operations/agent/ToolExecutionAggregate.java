...
import org.wso2.carbon.esb.module.ai.operations.Agent;
...
public class ToolExecutionAggregate extends TimerTask {
    ...
    private Agent agentMediator = null;
     ...
     * @param toolCount the minimum number of messages to be aggregated
     * @param mediator
     * @param faultHandler
     */
    public ToolExecutionAggregate(SynapseEnvironment synEnv, String corelation, long timeoutMillis, int toolCount,
    ...
    public synchronized boolean isComplete(SynapseLog synLog) {
        ...
        if (!completed) {
            ...
            if (!messages.isEmpty()) {
                ...
                Object prop = mc.getProperty(AgentConstant.AGENT_TOOL_EXECUTION +
                            (agentMediator.getId() != null ? "." + agentMediator.getId() : ""));
            ...
            if (toolCount > 0 && messages.size() >= toolCount) {
                if (synLog.isTraceOrDebugEnabled()) {
                    synLog.traceOrDebug(
