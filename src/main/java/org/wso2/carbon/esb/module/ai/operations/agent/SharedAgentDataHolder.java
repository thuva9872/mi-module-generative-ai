...
public class SharedAgentDataHolder extends SharedDataHolder {
    ...
    public void addToMemory(ChatMessage message) {
        ...
        if(message instanceof  AiMessage && ((AiMessage) message).hasToolExecutionRequests() && ((AiMessage) message).text() == null) {
            AiMessage aiMessage = (AiMessage) message;
            AiMessage toolExecutionRequestMessage = AiMessage.from("Tool Execution Request", aiMessage.toolExecutionRequests());
            aiServiceContext.chatMemory(memoryId).add(toolExecutionRequestMessage);
        }
        else {
            log.info("Message added to the memory: " + message.type() + ":: " + message.text());
